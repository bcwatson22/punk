...
const buildTileMarkup = (data) => {
  ...
  return new Promise(resolve => {
    ...
    const markup = data.map(beer => {
      ...
      const { name, abv, image_url, description, colour } = beer;

      return `
        <div class="tiles__tile" data-tiles-image="${image_url}"${colour ? ' style="--colour: ' + colour + '"' : ''}>
...
const renderTiles = (data) => buildTileMarkup(data).then(markup => injectHTML($wrapper, markup));
  ...
  $tiles.forEach(($tile, i) => {
    ...
    Vibrant
      ...
      .then(palette => {
        ...
        const colour = palette.Vibrant.hex;

        setCSSVar('colour', palette.Vibrant.hex, $tile)
        beers[i].colour = colour;
